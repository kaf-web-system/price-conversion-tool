...
export default function App() {
  ...
  const downloadCheck = () => {
    if (!data) return;
    const lines = ['sku,asin,商品名,商品説明,商品仕様,現在価格,改定後価格,差額,マッチ,長さm,個数,ケーブル加算,プラグ加算,プラグ内訳,手動対応理由,ケーブル単価未登録'];
    for (const r of data.results) {
  ...
  // ── ルールセクション（Amazon/Shopify共通・ReactNodeとして組み立て） ──
  const rulesSection = (
    <section style={card}>
      <h2>2. キーワードと加算額を設定</h2>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, padding: '8px 12px', background: '#f5f8ff', border: '1px solid #c8d8f0', borderRadius: 4, flexWrap: 'wrap' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
        ...
        <tbody>
          {rules.map((r, i) => (
            <tr key={r.id}>
              <td style={td}><input style={inp} value={r.label} onChange={(e) => updateRule(i, { label: e.target.value })} /></td>
              <td style={td}><input style={inp} value={r.pattern} onChange={(e) => updateRule(i, { pattern: e.target.value })} /></td>
              <td style={td}><input type="number" style={inp} value={r.cablePerMeter} onChange={(e) => updateRule(i, { cablePerMeter: Number(e.target.value) })} /></td>
              <td style={td}><input type="number" style={inp} value={r.plugPerPiece} onChange={(e) => updateRule(i, { plugPerPiece: Number(e.target.value) })} /></td>
              <td style={td}><button onClick={() => removeRule(i)} style={btnDanger}>削除</button></td>
            </tr>
          ))}
        </tbody>
      </table>
      <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
        <button onClick={addRule} style={btnSecondary}>＋ ルール追加</button>
        <button onClick={clearAllRules} style={{ ...btnSecondary, color: '#c00', borderColor: '#fcc' }}>全クリア</button>
        <span style={{ fontSize: 12, color: '#999', marginLeft: 4 }}>ルールは自動保存されます（{rules.length}件）</span>
      </div>
    </section>
  );
  ...
  return (
    <main style={{ maxWidth: 1100, margin: '0 auto', padding: 24, fontFamily: 'sans-serif', background: '#fff', color: '#000', minHeight: '100vh' }}>
      ...
      {tab === 'price' && (
        <>
          <h1 style={{ margin: '0 0 6px' }}>価格改定ツール（プロトタイプ）</h1>
          ...
          {/* プラットフォーム切替 */}
          <div style={{ display: 'flex', gap: 0, marginBottom: 4, borderBottom: '1px solid #ddd' }}>
            {([['amazon', 'Amazon'], ['shopify', 'Shopify']] as [Platform, string][]).map(([key, label]) => (
              <button
                ...
                style={{
                  ...
                  background: 'none', border: 'none',
                  borderBottom: platform === key ? '2px solid #0070f3' : '2px solid transparent',
                  marginBottom: -1,
                  color: platform === key ? '#0070f3' : '#666',
                  cursor: 'pointer',
                }}
              >
                {label}
                ...
                {diagInfos.map((d, i) => (
                  <div key={i} style={{ marginBottom: 16, padding: '10px 14px', background: d.error ? '#fff5f5' : '#f8fff8', border: `1px solid ${d.error ? '#fcc' : '#cec'}`, borderRadius: 4, fontSize: 13 }}>
                    ...
                    <table style={{ borderCollapse: 'collapse', fontSize: 12 }}>
                      <tbody>
                        <tr><td style={diagTh}>ワークブック内シート</td><td style={diagTd}>{d.sheetNames.length === 0 ? '（取得できず）' : d.sheetNames.map((s, j) => (<span key={j} style={{ marginRight: 8, background: s === 'テンプレート' ? '#d0f0d0' : '#eee', padding: '1px 6px', borderRadius: 3 }}>{s}</span>))}</td></tr>
                        <tr><td style={diagTh}>「テンプレート」シート</td><td style={diagTd}>{d.hasTemplateSheet ? '見つかった ✅' : '見つからなかった ❌'}</td></tr>
                        <tr><td style={diagTh}>実データ行数</td><td style={diagTd}>{d.dataRowCount.toLocaleString()} 件</td></tr>
                        <tr><td style={diagTh}>SKU列</td><td style={diagTd}>{d.columns.sku !== null ? `列インデックス ${d.columns.sku} で検出` : '未検出'}</td></tr>
                        <tr><td style={diagTh}>商品名列</td><td style={diagTd}>{d.columns.name !== null ? `列インデックス ${d.columns.name} で検出` : '未検出'}</td></tr>
                        <tr><td style={diagTh}>販売価格列</td><td style={diagTd}>{d.columns.price !== null ? `列インデックス ${d.columns.price} で検出` : '未検出'}</td></tr>
                        <tr><td style={diagTh}>ステータス列</td><td style={diagTd}>{d.columns.status !== null ? `列インデックス ${d.columns.status} で検出` : '未検出'}</td></tr>
                        {d.error && <tr><td style={{ ...diagTh, color: '#c00' }}>エラー</td><td style={{ ...diagTd, color: '#c00', whiteSpace: 'pre-wrap' }}>{d.error}</td></tr>}
                      </tbody>
                    </table>
                  </div>
                ))}
            ...
            {data && (
              <section style={card}>
                ...
                <p style={{ margin: '0 0 14px' }}>
                  ...
                  自動改定: <b style={{ color: '#047857' }}>{data.autoCount.toLocaleString()}</b> 件 ／
                  手動対応: <b style={{ color: '#b45309' }}>{data.manualCount.toLocaleString()}</b> 件
                  （自動率 {stats?.rate}%）
                </p>
                {(stats?.cableUnregisteredCount ?? 0) > 0 && (
                  <p style={{ color: '#c00', fontSize: 13, margin: '-8px 0 14px' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: '#f0f0f0', position: 'sticky', top: 0 }}>
                        <th style={th}>SKU</th><th style={th}>商品名</th><th style={th}>商品説明</th>
                        <th style={th}>商品仕様</th><th style={th}>マッチ</th><th style={th}>長さ(m)</th>
                        <th style={th}>個数</th><th style={th}>現価格</th><th style={th}>新価格</th>
                        <th style={th}>差額</th><th style={th}>手動理由</th><th style={th}>ケーブル単価未登録</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.preview.map((r, i) => (
                        <tr key={i} style={{ background: r.newPrice === null ? '#fff8e1' : r.cableRateUnregistered ? '#fff0f0' : 'transparent' }}>
          ...
          {/* Shopify: 1→2→3→診断→4（rulesSection を ShopifyTool に渡して順序保証） */}
...
const btnSecondary: React.CSSProperties = { padding: '5px 14px', background: '#fff', color: '#333', border: '1px solid #ccc', borderRadius: 4, cursor: 'pointer', fontSize: 13 };
